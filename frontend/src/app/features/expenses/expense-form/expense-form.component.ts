...
export class ExpenseFormComponent implements OnInit {
  ...
  get sharedIsCartao(): boolean {
    return this.sharedForm.value.payment_method === 'cartao';
  }
  ...
  loadRecurringTemplates(): void {
    this.expenseService
      ...
      .subscribe({ next: (t) => this.recurringTemplates.set(t) });
  }
  ...
  deleteRecurring(id: number): void {
    ...
    this.expenseService
      ...
      .subscribe({ next: () => this.loadRecurringTemplates() });
  }
  ...
  generateCurrentMonth(): void {
    ...
    this.expenseService
      .generateMonthRecurring(now.getMonth() + 1, now.getFullYear())
      .subscribe({
        next: (res) => {
  ...
  form = this.fb.group({
    ...
    installments: [
      ...
      [Validators.min(2), Validators.max(60)],
    ],
  ...
  get isInstallment(): boolean {
    return !!this.form.value.is_installment;
  }
  ...
  ngOnInit(): void {
    ...
    this.sharedDebtSvc
      .listGroups()
      .subscribe({ next: (g) => this.sharedGroups.set(g) });
  ...
  private loadSelects(): void {
    ...
    this.cardService
      .list()
      .subscribe({ next: (cards) => this.cards.set(cards) });
  }
  ...
  isSharedParticipant(id: number): boolean {
  ...
  submit(): void {
      ...
      this.expenseService.delete(this.editId()!).subscribe({
          ...
          this.expenseService.create(payload).subscribe({
            ...
            error: (err) => {
              ...
              this.errorMessage.set(
                this.toast.apiError(
                  err,
                  'Erro ao criar parcelas. Tente novamente.',
                ),
              );
            },
          });
        ...
        error: (err) => {
          ...
          this.errorMessage.set(
            this.toast.apiError(
              err,
              'Erro ao remover gasto original. Tente novamente.',
            ),
          );
        },
      });
    ...
    req$.subscribe({
      next: () => {
        this.toast.success(
          this.isEdit() ? 'Gasto atualizado.' : 'Gasto criado.',
        );
  ...
  installmentPreview = computed(() => {
    const v = this.form.getRawValue();
    if (!v.is_installment || !v.amount || (v.installments ?? 1) < 2)
      return null;
